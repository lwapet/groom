...
public class UseChecker extends AbstractStmtSwitch {
  ...
  public void caseAssignStmt(AssignStmt stmt) {
    /// processing a statement; can call the useVisitor to insert new cast
    ...
    if (rhs instanceof Local) {
      ...
      if (this.tg.get(base) instanceof ArrayType) {
        ...
        // For some fixed type T, we assume that we can fix the array to T[]. wrong!!!!!!!
        if (bt instanceof RefType || bt instanceof NullType) {
          RefType rt = bt instanceof NullType ? null : (RefType) bt;//-------object
          if (rt == null || rt.getSootClass().getName().equals("java.lang.Object")
            ...
            //added by lavoisier
            if(jb.getMethod().getDeclaringClass().getName().contains("com.google.android.gms.internal.firebase-perf.yd") && jb.getMethod().toString().contains("a(int,java.lang.Object[])")){
              System.out.println("we want to  cast "+ 
              "  the instruction "  + stmt);
            }
            //end
            outer: for (UnitValueBoxPair usePair : uses.getUsesOf(stmt)) {
              ...
              //added by lavoisier
              if(jb.getMethod().getDeclaringClass().getName().contains("com.google.android.gms.internal.firebase-perf.yd") && jb.getMethod().toString().contains("a(int,java.lang.Object[])")){
                System.out.println("we are processing the statement " + useStmt);
              }
              //end
              ...
              if (useStmt.containsInvokeExpr()) {
                //added by lavoisier
                if(jb.getMethod().getDeclaringClass().getName().contains("com.google.android.gms.internal.firebase-perf.yd") && jb.getMethod().toString().contains("a(int,java.lang.Object[])")){
                  System.out.println("it contains invoke expression");
                }
                //end
              ...
              // if the other value is a primitive.
                ...
                //added by lavoisier
                if(jb.getMethod().getDeclaringClass().getName().contains("com.google.android.gms.internal.firebase-perf.yd") && jb.getMethod().toString().contains("a(int,java.lang.Object[])")){
                  System.out.println("it contains if expression");
                }
                //end
                ...
                AssignStmt useAssignStmt = (AssignStmt) useStmt;
                 //added by lavoisier
                if(jb.getMethod().getDeclaringClass().getName().contains("com.google.android.gms.internal.firebase-perf.yd") && jb.getMethod().toString().contains("a(int,java.lang.Object[])")){
                  System.out.println("it is an assignement expression");
                }
                //end
      ...
      if(jb.getMethod().getDeclaringClass().getName().contains("com.google.android.gms.internal.firebase-perf.yd") && jb.getMethod().toString().contains("a(int,java.lang.Object[])")){
        System.out.println("final type retain is " + trhs);
      }
