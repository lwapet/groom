 ...
 * @since 26-Jan-2006
 */
public class ClassRenamer extends SceneTransformer {
	...
	private final Set<String> broadcastReceivers;
	private final Set<String> components;

	public final HashMap<String, String> componentMappings = new HashMap<>();

	public ClassRenamer(ProcessManifest manifest) {
		components = new HashSet<>();
		activities = manifest.getAllActivities()
				.stream()
		...
		broadcastReceivers = manifest.getReceivers()
				...
				.map(n -> (String) n.getAttribute("name").getValue())
				.collect(toSet());
		components.addAll(broadcastReceivers);
				...
				for (Unit u : aBody.getUnits()) {
					for (ValueBox vb : u.getUseAndDefBoxes()) {
						...
						if (v instanceof ClassConstant) {
							...
							Type type = constant.toSootType();
							vb.setValue(ClassConstant.fromType(type));
	...
	public String getOrAddNewName(final String packageName, final String className) {
		...
		if (className != null) {
			...
			if (oldToNewClassNames.containsKey(newFqn)) {
				String o = oldToNewClassNames.get(newFqn);
